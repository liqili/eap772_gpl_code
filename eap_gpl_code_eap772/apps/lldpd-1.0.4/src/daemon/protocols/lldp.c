...
static LLDP_ENTRY l_lldpEntry;
extern LLDP_INFO g_lldpInfo;
extern int g_lldpShmId;
extern int g_semid;

char* lldp_get_str_chassis(struct lldpd_chassis *chassis)
{
	char *pChassis = NULL; 
	size_t len = 0;
	...
	case LLDP_CHASSISID_SUBTYPE_LOCAL:
		strncpy(l_lldpEntry.mac, chassis->c_id, sizeof(l_lldpEntry.mac));
		break;
	case LLDP_CHASSISID_SUBTYPE_LLADDR:
		snprintf(l_lldpEntry.mac, 18, "%02X-%02X-%02X-%02X-%02X-%02X",(uint8_t)(chassis->c_id)[0], (uint8_t)(chassis->c_id)[1], (uint8_t)(chassis->c_id)[2], (uint8_t)(chassis->c_id)[3], (uint8_t)(chassis->c_id)[4], (uint8_t)(chassis->c_id)[5]);
		break;
	case LLDP_CHASSISID_SUBTYPE_ADDR:
		...
		if (len > 0) {
			pChassis = (char *)calloc(1, len - 1);
			if (!pChassis)  { 
				l_lldpEntry.mac[0] = '\0';
				break;
			}
			memset(pChassis, 0, len - 1);
			if (inet_ntop((chassis->c_id[0] == LLDP_MGMT_ADDR_IP4)?
				AF_INET:AF_INET6,
				&chassis->c_id[1], pChassis, len) == NULL)
			{
				free(pChassis);
				break;
			}
			strncpy(l_lldpEntry.mac, pChassis, sizeof(l_lldpEntry.mac));
			free(pChassis);
		}
		break;	
	default:
	...
	return l_lldpEntry.mac;
}

char* lldp_get_str_port(struct lldpd_port *port)
{ 
	char *pPort = NULL; 
	size_t len = 0;
	...
	case LLDP_PORTID_SUBTYPE_LOCAL:
		strncpy(l_lldpEntry.port, port->p_id, sizeof(l_lldpEntry.port));
		break;
	case LLDP_PORTID_SUBTYPE_LLADDR:
		snprintf(l_lldpEntry.port, 18, "%02X-%02X-%02X-%02X-%02X-%02X",(uint8_t)(port->p_id)[0], (uint8_t)(port->p_id)[1], (uint8_t)(port->p_id)[2], (uint8_t)(port->p_id)[3], (uint8_t)(port->p_id)[4], (uint8_t)(port->p_id)[5]);
		break;
	...
	if (len > 0) {
		pPort = (char *)calloc(1, len - 1);
		if (!pPort) { 
			l_lldpEntry.port[0] = '\0';
			break;
		}
		memset(pPort, 0, len - 1);
		if (inet_ntop((port->p_id[0] == LLDP_MGMT_ADDR_IP4)?
			AF_INET:AF_INET6,
			&port->p_id[1], pPort, len) == NULL)
		{
			free(pPort);
			break;
		}
		strncpy(l_lldpEntry.port, pPort, sizeof(l_lldpEntry.port));
		free(pPort);
	}
		break;
	default:
	...
	return l_lldpEntry.port;
}

static void lldp_write(struct lldpd_chassis *chassis, struct lldpd_port *port, struct lldpd_hardware *hardware)
{
	unsigned char *pShBuf = NULL;
	int i = 0, j = 0;
	
	if (!port || !chassis || !hardware)
	{
		log_warn("lldp", "input is NULL");
		return;
	}

	memset(&l_lldpEntry, 0, sizeof(l_lldpEntry));
	lldp_get_str_chassis(chassis);
	lldp_get_str_port(port);
	
	if (0 == port->p_ttl)
	{
		log_debug("lldp", "this lldp inform is expired");
		return;
	}

	if (g_lldpInfo.lanPortNum >= MAX_LAN_PORT_NUM)
	{
		log_warn("lldp", "the lldp lan port number exceeds the max bumber");
		return;
	}	
	
	/* connect to share mem */
	if (NULL == (pShBuf = shmat(g_lldpShmId, NULL, 0)))
	{
		log_warn("lldp", "failed to connect share mem");
		return;
	}

	/* copy to share mem */
	if (-1 == util_sem_lock(g_semid))
	{
		log_warn("lldp", "failed to copy to share mem");
		goto leave;
	}
	
	for (i = 0; i < g_lldpInfo.lanPortNum && g_lldpInfo.lanPortNum < MAX_LAN_PORT_NUM; i++)
	{
		if (!strncmp(g_lldpInfo.lldpList[i].ulPortIfName, hardware->h_ifname, PORT_ALIAS_NAME_LEN))
		{
			if (g_lldpInfo.lldpList[i].neighborNum >= LLDPD_MAX_NEIGHBORS)
			{
				log_warn("lldp", "the lldpEntry number exceeds the max neighbors");
				util_sem_unlock(g_semid);
				goto leave;
			}

			for (j = 0; j < g_lldpInfo.lldpList[i].neighborNum && g_lldpInfo.lldpList[i].neighborNum < LLDPD_MAX_NEIGHBORS; j++)
			{
				if ((strncmp(l_lldpEntry.mac, g_lldpInfo.lldpList[i].lldpEntry[j].mac, MAX_LLDP_LENGTH) == 0) &&
					(strncmp(l_lldpEntry.port, g_lldpInfo.lldpList[i].lldpEntry[j].port, MAX_LLDP_LENGTH) == 0))
				{
					log_debug("lldp", "duplicate l_lldpEntry, no need to write");
					util_sem_unlock(g_semid);
					goto leave;
				}
			}
			
			memcpy(&g_lldpInfo.lldpList[i].lldpEntry[j], &l_lldpEntry, sizeof(LLDP_ENTRY));
			g_lldpInfo.lldpList[i].neighborNum++;		
			break;
		}
	}

	if ((i == g_lldpInfo.lanPortNum) && (g_lldpInfo.lanPortNum < MAX_LAN_PORT_NUM))
	{
		memset(&g_lldpInfo.lldpList[i], 0, sizeof(LLDP_LIST));
		strncpy(g_lldpInfo.lldpList[i].ulPortIfName, hardware->h_ifname, PORT_ALIAS_NAME_LEN);
		memcpy(&g_lldpInfo.lldpList[i].lldpEntry[j], &l_lldpEntry, sizeof(LLDP_ENTRY));
		g_lldpInfo.lldpList[i].neighborNum++;
		g_lldpInfo.lanPortNum++;
	}
	
	memcpy(pShBuf, &g_lldpInfo, sizeof(g_lldpInfo));
	util_sem_unlock(g_semid);
	
leave:
/* disconnect from share mem */
if (g_lldpShmId != -1)
{
	shmdt(pShBuf);
}
...
#ifdef CONFIG_TRUNK_SUPPORT
	      POKE_UINT8((port->p_aggregid) ? 3:1) &&
#else
	      POKE_UINT8(0) &&
#endif /* CONFIG_TRUNK_SUPPORT */
	  ...
	  struct lldpd_hardware *hardware)
{
	...
	if(strncmp(hardware->h_ifname, "eth", 3))
		return 0;
	memcpy(chassis->c_id, port->p_id, port->p_id_len);
    ...
    struct lldpd_chassis **newchassis, struct lldpd_port **newport)
{
	...
	int length, gotend = 0, ttl_received = 0, chassis_received = 0, port_received = 0;
	int tlv_size, tlv_type, tlv_subtype;
	int port_descr = 0, system_name = 0, system_descr = 0, system_cap = 0, dot1_pvid = 0,
		dot3_mac = 0, dot3_la = 0, dot3_mfs = 0, dot3_power = 0;
	u_int8_t *pos, *tlv;
	char *b = NULL;
#ifdef ENABLE_DOT1
	...
	/* Check for invalid lldp packets(fix bug 701061、708463) */	
	if ((chassis_received > 1) || (port_received > 1) || (ttl_received > 1) || (port_descr > 1) || (system_name > 1) ||
		(system_descr > 1)|| (system_cap > 1)||(dot1_pvid > 1) || (dot3_mac > 1) || (dot3_power > 1) || (dot3_la > 1) ||
		(dot3_mfs > 1))
	{
		log_warnx("lldp", "some invalid tlv for frame received on %s",
		    hardware->h_ifname);
		goto malformed;
	}
	*newchassis = chassis;
	*newport = port;
	...
	lldp_write(chassis, port, hardware);
