...
iflinux_get_permanent_mac(struct lldpd *cfg,
    ...
    struct interfaces_device *iface)
{
	return; /* 端口聚合下会获取到permanet mac，导致发包源mac错误 2023.3.21 by tanlei */
	struct interfaces_device *master = iface->upper;
    ...
    struct interfaces_device_list *interfaces)
{
	...
	TAILQ_FOREACH(iface, interfaces, next) {
		if ((iface->type & IFACE_VLAN_T) && (!strncmp(iface->name, "eth", 3)))
		{
			g_isVlanPort = 1;
			if (iface->type & (
				IFACE_BOND_T|IFACE_BRIDGE_T))
		...
		if (iface->lower && (!iface->driver || (strcmp(iface->driver, "veth") && strcmp(iface->driver, "dsa"))) && strncmp(iface->name, "eth", 3)) {
