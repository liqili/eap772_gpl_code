...
LLDP_INFO g_lldpInfo;
int g_lldpShmId = -1;
int g_semid = -1;

int util_sem_lock(int semid)
{
    int ret = -1;
    int i   = 0;
    ...
    for (i = 0; i < 30 && ret; i++)
    {
        ...
        if (ret != 0)
        {
            ...
            {
                printf("lock interrupted by signal\n");
                ret = -1;
            }
            else if (errno == EAGAIN || ret == EAGAIN)
            {
                printf("timed out, reason =%s rc=%d\n", strerror(errno), ret);
                ret =  -1;
                break;
            }
            else
            {
                printf("lock failed, reason=%s rc=%d\n", strerror(errno), ret);
                ret = -1;
                break;
            }
        }
...
int util_sem_unlock(int semid)
{
    ...
    {
        printf("release lock failed, reason=%s\n", strerror(errno));
        ret = -1;
    }
...
int util_init_sem(int *pSemid, unsigned int sem_key)
{
	int flags = 0;
	...
	if ((*pSemid = semget(sem_key, 0, flags)) == -1)
    {
        if ((*pSemid = semget(sem_key, 1, flags)) == -1)
        {
            printf("semget failed, reason: %s\n", strerror(errno));
            return -1;
        }
         ...
         */
        if(semctl(*pSemid, 0, SETVAL, 1) == -1)
        {
            printf("setctl setval 1 failed, reason: %s\n", strerror(errno));
			if (semctl(*pSemid, 0, IPC_RMID) < 0)
			{
				printf("semctl IPC_RMID failed, reason: %s\n", strerror(errno));
			}
            return -1;
        }
    }
	...
	for (hardware = TAILQ_FIRST(&cfg->g_hardware); hardware != NULL;
		...
		if (!hardware->h_flags) {
			...
			for (port = TAILQ_FIRST(&hardware->h_rports); port != NULL; port = port_next)
			{
				...
				if ((now >= port->p_lastupdate + port->p_ttl) || !(hardware->h_flags & IFF_RUNNING))
				{
					/* connect to share mem */
					if (NULL == (pShBuf = shmat(g_lldpShmId, NULL, 0)))
					{
						log_warn("event", "failed to connect share mem");
						return;
					}
					...
					memset(&lldpEntry, 0, sizeof(LLDP_ENTRY));
					strncpy(lldpEntry.mac, lldp_get_str_chassis(port->p_chassis), MAX_LLDP_LENGTH);
					strncpy(lldpEntry.port, lldp_get_str_port(port), MAX_LLDP_LENGTH);
					
					for (i = 0; i < g_lldpInfo.lanPortNum; i++)
					{
						if (!strncmp(g_lldpInfo.lldpList[i].ulPortIfName, hardware->h_ifname, PORT_ALIAS_NAME_LEN))
						{
							for (j = 0; j < g_lldpInfo.lldpList[i].neighborNum; j++)
							{
								if ((strncmp(lldpEntry.mac, g_lldpInfo.lldpList[i].lldpEntry[j].mac, MAX_LLDP_LENGTH) == 0) &&
									(strncmp(lldpEntry.port, g_lldpInfo.lldpList[i].lldpEntry[j].port, MAX_LLDP_LENGTH) == 0))
								{
									if(g_lldpInfo.lldpList[i].neighborNum > 1)
									{
										memmove(&g_lldpInfo.lldpList[i].lldpEntry[j], &g_lldpInfo.lldpList[i].lldpEntry[j+1], (g_lldpInfo.lldpList[i].neighborNum - j - 1) * sizeof(LLDP_ENTRY));
									}
									else
									{
										memset(&g_lldpInfo.lldpList[i].lldpEntry[j], 0, sizeof(LLDP_ENTRY));
									}
							...
							if (g_lldpInfo.lldpList[i].neighborNum == 0)
							{
								if(g_lldpInfo.lanPortNum > 1)
								{
									memmove(&g_lldpInfo.lldpList[i], &g_lldpInfo.lldpList[i+1], (g_lldpInfo.lanPortNum - i - 1) * sizeof(LLDP_LIST));
								}
								else
								{
...
lldpd_main(int argc, char *argv[], char *envp[])
{
	...
	memset(&g_lldpInfo, 0, sizeof(g_lldpInfo));
	/* initial shared men */
	if ((g_lldpShmId = shmget(LLDP_SHMEM_KEY_ID, 0, 0)) < 0)
	{
		log_info("main", "shmget failed");
		free(cfg);
		return (0);
	}

	/* initial sem */
	if ((g_semid = semget(LLDP_SEM_KEY_ID, 0, 0)) == -1)
	{
		log_info("main", "semget failed");
		free(cfg);
		return (0);
	}
