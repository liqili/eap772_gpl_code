    ...
    struct interfaces_device *iface)
{
...
#ifdef CONFIG_MULTIPORT_SUPPORT
	int i = 0, j = 0;
	EAP_SWITCH_PORT_CONFIG portCfg;
#else
	char ifname[IFNAMELEN] = {0};
#endif /* CONFIG_MULTIPORT_SUPPORT */
	char upAliasName[DEVCFG_PORT_ALIAS_NAME_LEN] = {0};
	char downAliasName[DEVCFG_PORT_ALIAS_NAME_LEN] = {0};
	...
	*/
	...
	if (portid_type == LLDP_PORTID_SUBTYPE_IFNAME ||
		...
		DEVCFG_ULPORT_LOGICAL_IF_NAME(portCfg.uplinkPort.ifname, IFNAMELEN);

		if (!strcmp(port->p_descr, portCfg.uplinkPort.ifname))
		{
			DEVCFG_ULPORT_ALIAS_NAME(upAliasName, DEVCFG_PORT_ALIAS_NAME_LEN);
			free(port->p_descr);
			port->p_descr = strdup(upAliasName);
		}
		else
		{
			for (i = 0; i < DEVCFG_DLPORT_NUM(); i++)
			{
				DEVCFG_DLPORT_LOGICAL_IF_NAME(i + 1, portCfg.downlinkPort[i].ifname, IFNAMELEN);
				if (!strcmp(port->p_descr, portCfg.downlinkPort[i].ifname))
				{
					DEVCFG_DLPORT_ALIAS_NAME(i+1, downAliasName, DEVCFG_PORT_ALIAS_NAME_LEN);
					free(port->p_descr);
					port->p_descr = strdup(downAliasName);
					break;
				}
			}
		}
#else
		/* 0 == DEVCFG_DLPORT_NUM() : only one upLinkPort
		   1 == DEVCFG_DLPORT_NUM() : one upLinkPort and one downlinkPort */
		if (0 == DEVCFG_DLPORT_NUM())
		{
			DEVCFG_ULPORT_ALIAS_NAME(upAliasName, DEVCFG_PORT_ALIAS_NAME_LEN);
			if (port->p_descr)
			{
		...
		else if (1 == DEVCFG_DLPORT_NUM())
		{
			DEVCFG_ULPORT_IFNAME(ifname, sizeof(ifname));
			if (!strcmp(port->p_descr, ifname))
			{
				DEVCFG_ULPORT_ALIAS_NAME(upAliasName, DEVCFG_PORT_ALIAS_NAME_LEN);
				if (port->p_descr)
				{
			...
			{
				DEVCFG_DLPORT_ALIAS_NAME(1, downAliasName, DEVCFG_PORT_ALIAS_NAME_LEN);
				if (port->p_descr)
				{
					free(port->p_descr);
				}
    ...
    int(*init)(struct lldpd *, struct lldpd_hardware *))
{
	...
	TAILQ_FOREACH(iface, interfaces, next) {
		...
		if (g_isVlanPort && (!strcmp(iface->name, "eth0") || !strcmp(iface->name, "eth1")))
		{
			continue;
		}
...
#if 0  /* 解决由于端口聚合导致lldp源mac错误的问题 by tanlei 2023.3.29*/
