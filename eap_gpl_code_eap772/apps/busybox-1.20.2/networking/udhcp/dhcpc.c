...
#ifdef TP_BUSYBOX_MODIFY
#include "unix_sock.h"
#include "unix_sock_msgDef.h"
#include "opLan_info.h"
#endif /* TP_BUSYBOX_MODIFY */
...
#ifdef TP_BUSYBOX_MODIFY
uint32_t option_retrieve32(uint8_t **optionptr) {
	unsigned int val;
	...
	memcpy(&val, *optionptr, 4);
	*optionptr += 4;
	
	return ntohl(val);
}

void write_ac_addr(unsigned int *addr, unsigned short len)
{
	FILE *fp;
	int ii = 0;
	...
	for (ii = 0; ii < len; ii++){
		...
		memcpy(&a, &addr[ii], 4);
		strAddr = inet_ntoa(a);
		fwrite(strAddr,strlen(strAddr),1,fp);
		fwrite("\0\n",2,1,fp);
	}
...
int manage_option138_acmsg(uint8_t *optionptr, void **ret)
{
	uint8_t optlen = 0;
	uint8_t *tempptr = optionptr;
	int ipCount = 0, ii = 0;
	//struct acmsg_option_header acmsg;
	
	if (NULL == optionptr || NULL == ret){
		//PRINTF_ECHO("manage_option_acmsg NULL == optionptr || NULL == ret");
		return -1;
	}
	optlen = *(optionptr-1);
	if (optlen != 0 && (optlen%4) !=  0){
		//PRINTF_ECHO("manage_option_acmsg optlen(%d) < OPTION_HEADER_LEN_ACMSG", optlen);
		return -1;
	}
	ipCount = optlen/4;
	if (ipCount > 8)
	...
	(*ret) = malloc(ipCount * sizeof (unsigned int));
	if (NULL == (*ret)){
		return -1;
	}
	for (ii = 0; ii < ipCount; ii++){
		((uint32_t*)(*ret))[ii] = option_retrieve32(&tempptr);
	}
	return  ipCount ;
}
#endif /* TP_BUSYBOX_MODIFY */
...
{	
	...
	char *argv[3];
#ifdef TP_BUSYBOX_MODIFY
	MSG_CENTER_MSG msgCenter;
	LAN_DHCPC_FEEDBACK *pMsg = NULL;
	uint8_t *mask, *gw, *dns, *acIp;
	uint32_t dns_num = 0, i = 0;
	unsigned int *ac_addrs = NULL;
	int ac_ipCount = 0;
	...
	if (sizeof(LAN_DHCPC_FEEDBACK) > MSG_CENTER_MSG_DATA_LEN)
	{
		return;
	}

	pMsg = (LAN_DHCPC_FEEDBACK *)msgCenter.content;
	memset(&msgCenter, 0, sizeof(msgCenter));
	
	if (strcmp(name, "renew") == 0 || strcmp(name, "bound") == 0)
	{
		pMsg->status = DHCPC_STATUS_CONNECTED;
	}
	else if (strcmp(name, "release") == 0 || strcmp(name, "nak") == 0 || strcmp(name , "leaseover") == 0)
	{
		pMsg->status = DHCPC_STATUS_DISCONNECTED;
	}
	else if (strcmp(name, "fallback") == 0)
	{
		pMsg->status = DHCPC_STATUS_FALLBACK;
	}
	else
	{
		return;
	...
	if (DHCPC_STATUS_DISCONNECTED == pMsg->status || DHCPC_STATUS_FALLBACK == pMsg->status)
	{
		pMsg->ip = 0;
		pMsg->mask = 0;
		pMsg->gw = 0;
	...
	else
	{
		...
		pMsg->ip = ntohl(packet->yiaddr);

		if ((mask = udhcp_get_option(packet, DHCP_SUBNET)) != NULL)
		{
			memcpy(&(pMsg->mask), mask, 4);
			pMsg->mask = ntohl(pMsg->mask);
		}
		else
		{
			return;
		...
		if ((gw = udhcp_get_option(packet, DHCP_ROUTER)) != NULL)
		{
			memcpy(&(pMsg->gw), gw, 4);
			pMsg->gw = ntohl(pMsg->gw);
		}

		if ((dns = udhcp_get_option(packet, DHCP_DNS_SERVER)) != NULL)
		{
			dns_num = (*(dns - 1))/4;
			...
			for (i = 0; i < dns_num; i++)
			{
				if (i == 0)
				{
					memcpy(&(pMsg->dnsServerA), dns + 4 * i, 4);
					pMsg->dnsServerA = ntohl(pMsg->dnsServerA); 
				}
				else
				{
					memcpy(&(pMsg->dnsServerB), dns + 4 * i, 4);
					pMsg->dnsServerB = ntohl(pMsg->dnsServerB); 
				}
			}
#if 0
            {
                ...
                if (NULL != fp)
                {
                    fprintf(fp, "# Dynamic resolv.conf file for eap-cs generated by udhcpc\r\n");
                    fprintf(fp, "# DO NOT EDIT THIS FILE BY HAND\r\n");
                    for (i = 0; i < dns_num; i ++)
                    {
                        inaddr.s_addr = info->dns[i];
		...
		if ((acIp = udhcp_get_option(packet, DHCP_CAPWAP_ACMSG))) 
		{
			ac_ipCount = manage_option138_acmsg(acIp, &ac_addrs);
			if (ac_ipCount > 0)
			{
				write_ac_addr(ac_addrs,ac_ipCount);
				free(ac_addrs);
				ac_addrs = NULL;
			}
		}
	...
	msgCenter.opId = UCL_OPID_LAN_DHCPC_FEEDBACK;
	msgCenter.len = sizeof(LAN_DHCPC_FEEDBACK);
	
	unix_sock_sendDataToSrvWithoutResp("busybox-dhcpc", UNIX_SOCK_ID_MSG_CENTER, 
									   &msgCenter, sizeof(msgCenter), UNIX_MSG_CENTER_MSG);
#else
...
int udhcpc_main(int argc UNUSED_PARAM, char **argv)
{
	...
	int retval;
	fd_set rfds;
	char intfName[DHCPC_INTF_NAME_LEN] = {0};
	UNIX_SOCK_FD unixSockFd;
	int clientFd = -1;
	int dataLen = 0;
	DHCPC_MSG msg;
	int first = 1;
	#define FALLBACK_TIMEOUT	9
	/* Default options */
	IF_FEATURE_UDHCP_PORT(SERVER_PORT = 67;)
	IF_FEATURE_UDHCP_PORT(CLIENT_PORT = 68;)
	client_config.interface = intfName;
	...
	unix_sock_initSrv(UNIX_SOCK_ID_DHCPC, &unixSockFd, 1);
	while(1)
	{
		clientFd = accept(unixSockFd.fd, NULL, NULL);
		if (clientFd < 0)
		{
			continue;
		}

		if (unix_sock_recvHeader(clientFd, UNIX_MSG_DHCPC_MSG, &dataLen) != 0)
		{
			close(clientFd);
			clientFd = -1;
			continue;
		...
		if (unix_sock_recvData(clientFd, dataLen, (unsigned char *)&msg) != 0)
		{
			close(clientFd);
			clientFd = -1;
			continue;
		...
		close(clientFd);
		clientFd = -1;
		if (msg.cmd != DHCPC_START)
		{
			continue;
		}
		else
		{
			if (msg.info.dialInfo.hostName[0] != '\0')
			{
				client_config.hostname = alloc_dhcp_option(DHCP_HOST_NAME, msg.info.dialInfo.hostName, 0);
			}

			if (msg.info.dialInfo.intfName[0] != '\0')
			{
				strcpy(intfName, msg.info.dialInfo.intfName);
			}
			else
			{
				strcpy(intfName, "br0");
	...
	while (access("/tmp/ucliteok", F_OK) != 0)
	{
		sleep(1);
	}
	...
	for (;;) {
		...
		FD_SET(unixSockFd.fd, &rfds);
		if (unixSockFd.fd > max_fd)
		{
			max_fd = unixSockFd.fd;
		}
 ...
 leasefail:
			 ...
			 */
					...
					timeout=timeout-(timeout>>2);//change the time to 7/8 lease passed by YingbiaoLiu@20150915 for bug98071
					continue;
				}
			...
			case REBINDING:
				...
				udhcp_run_script(NULL, "leaseover");
				alarm(0);
				first = 1;
				state = INIT_SELECTING;
				client_config.first_secs = 0; /* make secs field count from 0 */
		...
		if (FD_ISSET(unixSockFd.fd, &rfds))
		{
			clientFd = accept(unixSockFd.fd, NULL, NULL);
			if (clientFd < 0)
			{
			...
			close(clientFd);
			clientFd = -1;
			if (msg.cmd == DHCPC_START || msg.cmd == DHCPC_RENEW)
			{
				client_config.first_secs = 0; /* make secs field count from 0 */
				already_waited_sec = 0;
				perform_renew();
...
out:
		...
		case SIGALRM:
			if (state != BOUND)
			{
				udhcp_run_script(NULL, "fallback");
			}
			continue;
		case SIGTERM:
