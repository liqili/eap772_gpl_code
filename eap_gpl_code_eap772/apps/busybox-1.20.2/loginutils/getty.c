...
static void init_tty_attrs(int speed)
{
	...
	/* disable ctsrts and software flow control, to avoid tty been stopped by ctsrts and STOP_CHAR(tty) */
	G.tty_attrs.c_cflag &= (~CRTSCTS);
...
static void finalize_tty_attrs(void)
{
	 ...
	 */
	/* G.tty_attrs.c_iflag |= IXON | IXOFF;*/ /* disable software flow control */
	if (G.eol == '\r') {
