 ...
 * Author		:	Chen JianFeng (chenjianfeng@tp-link.net)
 * Created Date	:	04/6/2013
 *
 * Modified History:
 * 
 ***************************************************************/
...
#define S44 21
 ...
 static char l_BitsToHex(unsigned int num)
 {
	 if (10 > num)
		 return '0' + num;
	 else
 ...
 static char l_BitsToHexUp(unsigned int num)
 {
	 if (10 > num)
		 return '0' + num;
	 else
 ...
 /*******************************************************************
 * cal_md5 - output the MD5 encrypt code  
 */
 void cal_md5(unsigned char* output, unsigned char* input, int len)
 {
	 char digest[MANAGE_USER_PWD_LENGTH];
	 char *pDigestOutput = output;
	 int k = 0;
	 md5_make_digest(digest, input, len);
	 for (k = 0; k < MANAGE_USER_PWD_LENGTH; k++)
	 {
		 *pDigestOutput = l_BitsToHexUp((((unsigned int)digest[k]) & 0x0f0) >> 4);
		 pDigestOutput++;
		 *pDigestOutput = l_BitsToHexUp((unsigned int)digest[k] & 0x0f);
		 pDigestOutput++;
	 }
	 *pDigestOutput = '\0';
	 return;
