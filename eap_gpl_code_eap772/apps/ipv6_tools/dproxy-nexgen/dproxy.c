...
int dns_init()
{
  ...
  if ((dns_sock < 0) || (dns_output_sock < 0))
  {
     debug_perror("Could not create socket");
     exit(1);
  } 

  /* let the socket only listen IPV6,or the samba will be infected */
  if (setsockopt(dns_sock, IPPROTO_IPV6, IPV6_V6ONLY, &on, sizeof(on)) < 0)
  {
  	 debug_perror("dns_init: setsockopt:set socket option failed");
  }

  if (setsockopt(dns_output_sock, IPPROTO_IPV6, IPV6_V6ONLY, &on, sizeof(on)) < 0)
  {
  	 debug_perror("dns_init: setsockopt:set socket option failed");
  }
