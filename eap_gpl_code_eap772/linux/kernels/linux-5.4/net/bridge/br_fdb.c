		   ...
		   const unsigned char *addr, u16 vid, bool added_by_user)
{
	...
	struct br_fdb_event fdb_event;
	struct br_tp_fdb_notify tpFdbNotify;
	...
	if (likely(fdb)) {
		...
		if (unlikely(fdb->is_local)) {
			...
			if (unlikely(source != fdb->dst && !fdb->is_sticky)) {
				memset(&tpFdbNotify, 0, sizeof(struct br_tp_fdb_notify));
				ether_addr_copy(tpFdbNotify.addr, addr);
				tpFdbNotify.orig_dev = fdb->dst->dev;
				tpFdbNotify.dev = source->dev;
			...
			if (unlikely(fdb_modified)) {
				__br_notify(RTNLGRP_NEIGH, RTM_UPDATEAST, &tpFdbNotify);
