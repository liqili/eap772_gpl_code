...
#else
	/* 按照需求改变记录log的格式：增加unix时间戳、减少不必要的字段缩减单条日志长度 */
	memset(&ts, 0, sizeof(ts));
	ktime_get_real_ts64(&ts);
	printk(KERN_SOH "%c[%10u.%09lu] %s MAC SRC=%pM",
	       '0' + loginfo->u.log.level, ts.tv_sec, ts.tv_nsec,
	       prefix, eth_hdr(skb)->h_source);
#endif
...
#else
		printk(" IP SRC=%pI4 IP DST=%pI4 IP proto=%d",
