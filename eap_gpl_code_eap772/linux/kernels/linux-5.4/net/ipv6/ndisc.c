...
static void ndisc_router_discovery(struct sk_buff *skb)
{
	...
	if (!rt && lifetime) {
		...
		{
			/* 写 /proc 下 gateway 的值 */
			snprintf(in6_dev->cnf.default_gateway, sizeof(in6_dev->cnf.default_gateway), "%pI6", &ipv6_hdr(skb)->saddr);
			/* 写 /proc 下 prefix_len 的值 */
			if (NULL != pinfo)
			{
				in6_dev->cnf.default_prefix_len = pinfo->prefix_len;
			}
		}
#endif /* CONFIG_TP_IPV6_FEATURE_SUPPORT */
