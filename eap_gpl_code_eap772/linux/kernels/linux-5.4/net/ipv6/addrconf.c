...
static struct ipv6_devconf ipv6_devconf __read_mostly = {
	...
	.disable_policy		= 0,
#ifdef CONFIG_TP_IPV6_FEATURE_SUPPORT
	.ndisc_mbit = -1,
	.ndisc_obit = -1,
	.sendrs =  1,
	.default_gateway = "",
	.default_prefix_len = 64,
	.lastest_ipv6_address = "",
#endif /* CONFIG_TP_IPV6_FEATURE_SUPPORT */
};
...
/*
 * 以下配置地址的方式均会调用这个配置 ipv6 的函数
 *   1. ifconfig br0 add xxxx:xxxx::xxxx
 *   2. 有状态 dhcpv6 拨号
 *   3. 无状态 SLAAC 拨号
 *   4. ioctl 配置地址
 *
 * 其它暂时未使用到的配置地址的方式也会调用这个函数：
 *   1. 配置临时 ipv6 地址
 *   2. 开机生成 link-local 地址
 *   3. 。。。。。。
 *
 * 总的来说，基本是配置 IPv6 地址都会调用这个函数
 * */
...
#ifdef CONFIG_TP_IPV6_FEATURE_SUPPORT
	if ((cfg->pfx != NULL) && !(ipv6_addr_type(cfg->pfx) & IPV6_ADDR_LINKLOCAL))
	{
		/* get the lastest ipv6 address to /proc/sys/net/ipv6/<device_name>/lastest_ipv6_address */
		snprintf(idev->cnf.lastest_ipv6_address, sizeof(idev->cnf.lastest_ipv6_address), "%pI6", cfg->pfx);
	}
#endif /* CONFIG_TP_IPV6_FEATURE_SUPPORT */
...
/* 
* 以下配置地址的方式均会调用这个配置 ipv6 的函数
*   1. ifconfig br0 del xxxx:xxxx::xxxx
*   2. 有状态 DHCPv6 ipv6 地址过期
*   3. 无状态 SLAAC 拨号 地址过期
*   4. ioctl 删除地址
*   5. 。。。。。。 
* 
...
#ifdef CONFIG_TP_IPV6_FEATURE_SUPPORT
	/* 判断删除的地址是否是最新获得的地址，是则将 lastest_ipv6_address 置 0 */
	if ( (NULL != ifp->idev) )
	{
		struct in6_addr ipv6_address;
		if (in6_pton(ifp->idev->cnf.lastest_ipv6_address, strlen(ifp->idev->cnf.lastest_ipv6_address), ipv6_address.in6_u.u6_addr8, -1, NULL) > 0)
		{
			if (ipv6_addr_equal(&ipv6_address, &ifp->addr))
			{
				memset(ifp->idev->cnf.lastest_ipv6_address, 0, sizeof(ifp->idev->cnf.lastest_ipv6_address));
			}
		}
	}
#endif /* CONFIG_TP_IPV6_FEATURE_SUPPORT */
...
#ifdef CONFIG_TP_IPV6_FEATURE_SUPPORT
		/*
		 * Bug 831372 - 样机开启ipv6地址获取，前端gateway设置slaac拨号，
		 * 当 gateway 在两个网络前缀之间来回切换时，页面无法显示最新的ipv6地址。
		 * 判断地址相同之前就更新 lastest_ipv6_address 文件	
		 * 
		 * 在 ifp 非空时，即收到 RA 报文配置的地址在此之前已经存在，才更新 lastest_ipv6_address
		 */
		if ((ifp != NULL) && (addr != NULL) && (!(ipv6_addr_type(addr) & IPV6_ADDR_LINKLOCAL)))
		{
			write_lock(&in6_dev->lock);
			/* get the lastest ipv6 address to /proc/sys/net/ipv6/<device_name>/lastest_ipv6_address */
			snprintf(in6_dev->cnf.lastest_ipv6_address, sizeof(in6_dev->cnf.lastest_ipv6_address), "%pI6", addr);
			write_unlock(&in6_dev->lock);
		}
#endif /* CONFIG_TP_IPV6_FEATURE_SUPPORT */
			    ...
			    void __user *buffer, size_t *lenp, loff_t *ppos)
{
	...
	struct inet6_ifaddr *pIfp = NULL;
	struct inet6_dev *pI6dev = NULL;
	struct in6_addr tmpAddr;

	if ( (NULL == buffer) || (NULL == lenp) || (NULL == ppos) || (NULL == ctl))
	{
	...
	if (0 != ret)
	{
		printk("failed to get user buffer from /proc.\r\n");
		return -1;
	}

	if (write)
	{
		pNet = (struct net *)ctl->extra2;
		pI6dev = (struct inet6_dev *)ctl->extra1;
		if (pValp == &pNet->ipv6.devconf_dflt->sendrs)
		{
			return 0;
		}

		memset(&tmpAddr, 0, sizeof(tmpAddr));
		read_lock(&pI6dev->lock);
		list_for_each_entry(pIfp, &pI6dev->addr_list, if_list) 
		{
			if (ipv6_accept_ra(pIfp->idev) && 0 == pIfp->idev->cnf.disable_ipv6)
			{
				if (ipv6_addr_type(&pIfp->addr) & IPV6_ADDR_LINKLOCAL)
				{
					tmpAddr = pIfp->addr;
				}
			}
		}
		read_unlock(&pI6dev->lock);
		ndisc_send_rs(pI6dev->dev, &tmpAddr, &in6addr_linklocal_allrouters);
		return 0;
	}
	return -1;
}
...
static const struct ctl_table addrconf_sysctl[] = {
	...
	{
		.procname	= "default_prefix_len",
		.data		= &ipv6_devconf.default_prefix_len,
		.maxlen		= sizeof(int),
		.mode		= 0644,
		.proc_handler 	= proc_dostring,
	},
	{
		.procname	= "lastest_ipv6_address",
		.data		= &ipv6_devconf.lastest_ipv6_address,
		.maxlen		= 64,
		.mode		= 0444,
		.proc_handler 	= proc_dostring,
	},
