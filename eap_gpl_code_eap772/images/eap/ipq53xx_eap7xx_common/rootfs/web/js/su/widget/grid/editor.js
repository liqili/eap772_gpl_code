...
$.su.Widget("editor", {
	...
	setSubmitForm: function(me, parm){
		var me = me || this;
		obj = me.get(0);
		obj.submitForm = parm[1];
		//obj.submitCallback = parm[2];
		//console.log(parm);	
		return me;
	},
	completeEdit: function(me){
		var me = me || this,
			...
			/*below:for radioID*/
			var	radioID = me.find("[name=\"radioID\"]");

    /*for cluster, do not delete it*/
		if (obj.submitForm != null){
			form = obj.submitForm;
		}
		...
		if (form.form("validate")){
			...
			if (editingId != "add"){
				...
				if (radioID.length != 0){
					$.extend(dataObj, {radioID:radioID[0].value})
				}
				store.update(editingId, dataObj, function(){
				...
				store.insert(len, dataObj, function(){
					grid.grid("prompt", true);
					me.editor("cancelEdit");
					$("button.btn-submit").attr("disabled",false);
				}, function(){
