...
$.su.Widget("grid", {
	...
	create: function(defaults, options){
		...
		me.each(function(i, obj){
			...
			radioID = obj.radioID!=undefined?obj.radioID:-1;
			...
			for (var index = 0; index < columns.length; index++){
				...
				inHTML +=				"<span class=\"content\">"+column.text+"</span>";
				inHTML +=				btnSort;
			...
			var lineHeight = parseInt($("td.grid-content-td").css("height"), 10);
			...
			if (editorObj && editorObj.isEditor){
				if (editorObj.editing === false){
					...
					btn_delete_id = setTimeout(function(){
						me.grid("runProgress");
						var data = radioID!=-1?{radioID:radioID}:{};
						store.remove([key], data, function(){
							me.grid("prompt", true);
						}, function(){
							me.grid("prompt", false);
						}, function(){
	...
	initRow: function(me, params){	//row-index, data 返回html值
		...
		for (var kndex = 0; kndex < len; kndex++){
			...
			switch (column.xtype){
				...
				case "settings":
					...
					inHTML +=			"<span class=\"icon\" title=\"Delete\"></span>";
		...
		if (actionFlag.length > 0){
			for (var index = 0; index < actionFlag.length; index++){
				...
				inputs.attr('id', 'grid_edit_'+dataIndex+'_'+dIndex);
				inputs.attr('index', dIndex);
				...
				for (var lndex = 0; lndex < items.length; lndex++){
					...
					if (xtype == "html"){
						...
						if(item.change != undefined){
							input[xtype]($.extend({}, item, propertyObj)).on('ev_change', item.change);
						}else{
		...
		for (var index = 0, len = operations.length; index < len; index++){
			...
			switch (operations[index]){
				...
				case "add-lm":
					inHTML +=	"<a href=\"javascript:void(0);\" class=\"operation-btn btn-add "+fst+" "+lst+"\">";
					inHTML +=		"<span class=\"icon\"></span>";
					inHTML +=		"<span class=\"text\">"+$.su.CHAR.OPERATION.ADDGROUP+"</span>";
					inHTML +=	"</a>";
				...
				case "add-rm":
					inHTML +=	"<a href=\"javascript:void(0);\" class=\"operation-btn btn-add "+fst+" "+lst+"\">";
					inHTML +=		"<span class=\"icon\"></span>";
					inHTML +=		"<span class=\"text\">"+$.su.CHAR.OPERATION.ADDMEMBER+"</span>";
					inHTML +=	"</a>";
				...
				case "add-lp":
					inHTML +=	"<a href=\"javascript:void(0);\" class=\"operation-btn btn-add "+fst+" "+lst+"\">";
					inHTML +=		"<span class=\"icon\"></span>";
					inHTML +=		"<span class=\"text\">"+$.su.CHAR.OPERATION.ADDPROFILE+"</span>";
					inHTML +=	"</a>";
				...
				case "add-rp":
					inHTML +=	"<a href=\"javascript:void(0);\" class=\"operation-btn btn-add "+fst+" "+lst+"\">";
					inHTML +=		"<span class=\"icon\"></span>";
					inHTML +=		"<span class=\"text\">"+$.su.CHAR.OPERATION.ADDPROITEM+"</span>";
					inHTML +=	"</a>";
	...
	initPaging: function(me, params){
		...
		var paging = $("<input type=\"hidden\" class=\"hidden\">").paging(options);
	...
	getMaxRules: function(me){
		var me = me || this;
		return me.get(0).maxRules || 0;
	},
	setMaxRules: function(me, params){
		var me = me || this;
		var maxRules = params[1]
		maxRules = maxRules === 0 ? -1 : maxRules
		return me.get(0).maxRules = maxRules;
	},
	...
	runProgress: function(me){
		...
		bar.css("width", "0px");
		progressbar.fadeIn(100, function(){
			bar.animate({
				width: "50%"
			}, 100);
		});
	...
	prompt: function(me, params){		//successed: true, false;	text为错误内容
		...
		progressbar.find("div.progressbar-value").animate({
			width: "100%"
		}, 150, function(){
    ...
    showColumn: function(e, args){
        if(!args instanceof Array || args.length !== 2) {
            throw new Error("Please use such as 【.grid('showColumn', 'tableKey')】")
        }
        var key = args[1];
		var container = this.dom().find('div.grid-container');
		container.children('style.column-hide-style[name="'+ key +'"]').remove();
	},

	hideColumn: function(dom, args){
        if(!args instanceof Array || args.length !== 2) {
            throw new Error("Please use such as 【.grid('hideColumn', 'tableKey')】")
        }
        var key = args[1] instanceof Array ? args[1] : [args[1]];
		var style = '';
		var id = '#' + dom[0].id + ' ';
		var container = this.find('div.grid-container')[0];
		var me = me || this;
		var	obj = me.get(0);
		var	columns = obj.columns;
				...
				$.each(key, function (index, item) {
					...
					for (var i = 0; i < columns.length; i++) {
						...
						style += '<style class="column-hide-style" name="'+item+'">';
						style += id + 'th.grid-header.'+item+', ';
						if (columnIndex !== -1) {
							style += id + 'td.grid-content-td-'+columnIndex+', ';
						}
						style += id + 'td.grid-content-td-'+item+', ';
						style += id + 'td.grid-tool-td-'+item;
						style += '{display: none;width: 0;}</style>';
				})
